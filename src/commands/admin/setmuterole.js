...
module.exports = class SetMuteRoleCommand extends Command {
  ...
  run(message, args) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Le `Rôle mute` a été mis à jour avec succès. ✅')
    ...
    if (args.length === 0) {
      ...
      return message.channel.send({ embeds: [embed.addFields({ name: 'Rôle', value: `${oldMuteRole} ➔ \`Aucun\`` })] });
    }
    ...
    message.channel.send({ embeds: [embed.addFields({ name: 'Rôle', value: `${oldMuteRole} ➔ ${muteRole}` })] });
  }
};