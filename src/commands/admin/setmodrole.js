...
module.exports = class SetModRoleCommand extends Command {
  ...
  run(message, args) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Le `mod role` a été mis à jour avec succès. ✅')
    ...
    if (args.length === 0) {
      ...
      return message.channel.send({ embeds: [embed.addFields({ name: 'Rôle', value: `${oldModRole} ➔ \`Aucun\`` })] });
    }
    ...
    message.channel.send({ embeds: [embed.addFields({ name: 'Rôle', value: `${oldModRole} ➔ ${modRole}` })] });
  }
};