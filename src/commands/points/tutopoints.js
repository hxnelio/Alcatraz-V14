...
module.exports = class ExplainPointsCommand extends Command {
  ...
  run(message) {
    ...
    if (!disabledCommands.includes('crown'))
      crown += `\nUtilisez la commande \`${prefix}crown\` pour des informations spécifiques au serveur.`;

    const embed = new EmbedBuilder()
      .setTitle('Points et couronne')
      .setThumbnail(message.guild.iconURL({ dynamic: true }))
      .addFields({ name: 'Gagner des points', value: earningPoints })
      .setFooter({ text: "© 2025 - Alcatraz | Projet open-source" })
      .setTimestamp()
      .setColor(0x2f3136);
    if (checkingPoints) embed.addFields({ name: 'Points de contrôle', value: checkingPoints });
    if (leaderboard) embed.addFields({ name: 'Le classement', value: leaderboard });
    embed.addFields({ name: 'La Couronne', value: crown });
