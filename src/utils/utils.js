...
async function getCaseNumber(client, guild, modLog) {
  ...
  const message = (await modLog.messages.fetch({ limit: 100 })).filter(m => m.member === guild.members.me &&
    m.embeds[0] &&
...
async function transferCrown(client, guild, crownRoleId) {
  ...
  if (
    ...
    crownChannel.permissionsFor(guild.members.me).has([PermissionFlagsBits.SendMessages, PermissionFlagsBits.EmbedLinks]) &&
    crownMessage
  ) {
    crownMessage = crownMessage
      ...
      .replace(/`?\?points`?/g, points); 
    crownChannel.send({ embeds: [new EmbedBuilder().setDescription(crownMessage).setColor(guild.members.me.displayHexColor)] });
  }
