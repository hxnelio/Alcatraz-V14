...
db.prepare(`
  CREATE TABLE IF NOT EXISTS economy (
    ...
    weekly_timestamp INTEGER,
    work_timestamp INTEGER,
    beg_timestamp INTEGER,
    works_count INTEGER DEFAULT 0 NOT NULL,
    begs_count INTEGER DEFAULT 0 NOT NULL
  );
`).run();
...
const economy = {
  ...
  // Selects
  selectRow: db.prepare('SELECT * FROM economy WHERE user_id = ?;'),
  selectMoney: db.prepare('SELECT money FROM economy WHERE user_id = ?;'),
  selectBank: db.prepare('SELECT bank FROM economy WHERE user_id = ?;'),
  selectDaily: db.prepare('SELECT daily_timestamp FROM economy WHERE user_id = ?;'),
  selectWeekly: db.prepare('SELECT weekly_timestamp FROM economy WHERE user_id = ?;'),
  selectWork: db.prepare('SELECT work_timestamp FROM economy WHERE user_id = ?;'),
  selectBeg: db.prepare('SELECT beg_timestamp FROM economy WHERE user_id = ?;'),
  selectWorksCount: db.prepare('SELECT works_count FROM economy WHERE user_id = ?;'),
  selectBegsCount: db.prepare('SELECT begs_count FROM economy WHERE user_id = ?;'),

  // Updates
  updateMoney: db.prepare(`
    INSERT INTO economy (user_id, money, bank, works_count, begs_count)
    VALUES (?, ?, 0, 0, 0)
    ON CONFLICT(user_id) DO UPDATE SET money = money + ?;
  `),
  updateBank: db.prepare(`
    INSERT INTO economy (user_id, money, bank, works_count, begs_count)
    VALUES (?, 0, ?, 0, 0)
    ON CONFLICT(user_id) DO UPDATE SET bank = bank + ?;
  `),
  subtractMoney: db.prepare(`
    INSERT INTO economy (user_id, money, bank, works_count, begs_count)
    VALUES (?, 0, 0, 0, 0)
    ON CONFLICT(user_id) DO UPDATE SET money = money - ?;
  `),
  subtractBank: db.prepare(`
    INSERT INTO economy (user_id, money, bank, works_count, begs_count)
    VALUES (?, 0, 0, 0, 0)
    ON CONFLICT(user_id) DO UPDATE SET bank = bank - ?;
  `),
  updateDaily: db.prepare(`
    INSERT INTO economy (user_id, money, bank, daily_timestamp, works_count, begs_count)
    VALUES (?, 0, 0, ?, 0, 0)
    ON CONFLICT(user_id) DO UPDATE SET daily_timestamp = ?;
  `),
  updateWeekly: db.prepare(`
    INSERT INTO economy (user_id, money, bank, weekly_timestamp, works_count, begs_count)
    VALUES (?, 0, 0, ?, 0, 0)
    ON CONFLICT(user_id) DO UPDATE SET weekly_timestamp = ?;
  `),
  updateWork: db.prepare(`
    INSERT INTO economy (user_id, money, bank, work_timestamp, works_count, begs_count)
    VALUES (?, 0, 0, ?, 1, 0)
    ON CONFLICT(user_id) DO UPDATE SET work_timestamp = ?, works_count = works_count + 1;
  `),
  updateBeg: db.prepare(`
    INSERT INTO economy (user_id, money, bank, beg_timestamp, works_count, begs_count)
    VALUES (?, 0, 0, ?, 0, 1)
    ON CONFLICT(user_id) DO UPDATE SET beg_timestamp = ?, begs_count = begs_count + 1;
  `),
  deleteUser: db.prepare('DELETE FROM economy WHERE user_id = ?;')
};
